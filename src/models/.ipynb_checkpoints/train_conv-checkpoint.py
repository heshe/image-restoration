...
from src.models.model import Decoder, Encoder, Model
    ...
    def train(self):
        ...
        for epoch in range(self.args.n_epochs):
            ...
            for i in tqdm.tqdm(range(int(train_X.shape[0] / self.args.batch_size))):
                ...
                if i % 100 == 1:
                    print("Average Loss: ", overall_loss / (i * self.args.batch_size))
            ...
            print(
                ...
                "complete!",
                "\tAverage Loss: ",
                overall_loss / (i * self.args.batch_size),
            )
        ...
        X_origin = get_rbg_from_lab(
            (X * 255).view(16, 224, 224), (Y * 255).view(16, 224, 224, 2), n=10
        )
        X_hat = get_rbg_from_lab(
            (X * 255).view(16, 224, 224), (X_hat * 255).view(16, 224, 224, 2), n=10
        )

        for i in range(X_origin.shape[0]):
            im = Image.fromarray(X_origin[i])
            im.save(
                f"/Users/heshe/Desktop/mlops/image-restoration/reports/figures/orig/img{i}.png"
            )
            im = Image.fromarray(X_hat[i])
            im.save(
                f"/Users/heshe/Desktop/mlops/image-restoration/reports/figures/recon/img{i}.png"
            )
