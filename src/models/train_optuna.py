...
#        self.ROOT = str(Path(__file__).parent.parent.parent)
        self.ROOT = "C:/Users/Asger/OneDrive/Skrivebord/DTU/Machine_Learning_Operations/image-restoration"
    ...
    def train(self,trial):
        ...
        #self.args.batch_size = trial.suggest_categorical('batch_size', [64, 128, 256, 512])
...
def init_hydra(config : DictConfig) -> None:
    ...
    study = optuna.create_study(direction="minimize",
                                pruner=optuna.pruners.MedianPruner(
                                       n_startup_trials=1, n_warmup_steps=4, 
                                       interval_steps=1)) #, sampler=optuna.samplers.GridSampler(search_space))
    
    study.optimize(trainer.train, n_trials=10) 
