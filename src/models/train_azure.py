...
class Trainer:
    ...
    def train(self):
        ...
        print("Loading Data...")
        train_path = run.input_datasets['image-resto-data']
        ...
        run.log('Epochs', np.int(args.n_epochs))
        run.log('Latent dim', np.int(args.latent_dim))
        run.log('Hidden dim', np.int(args.hidden_dim))
        run.log('Overall loss', np.float(overall_loss))
        print("Finish!!")

        # Generate reconstructions
        if args.make_reconstructions:
            model.eval()
            with torch.no_grad():
        ...
        if args.save_model:
            ...
            model_file = args.model_name + ".pkl"
            joblib.dump(value=model, filename=model_file)
            run.upload_file(
                name = model_file,
                path_or_stream = os.path.join(self.ROOT, "models")
            )
